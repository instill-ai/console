...
import { Nullable, env, useUser } from "../../lib";
...
export type TopbarProps = {
  ...
  enableQuery: boolean;
  orgRelatedDropdownContent?: React.ReactNode;
};
...
export const Topbar = ({
  ...
  accessToken,
  enableQuery,
  orgRelatedDropdownContent,
}: TopbarProps) => {
  const router = useRouter();
  const { entity } = router.query;

  const user = useUser({
    enabled: enableQuery && !!accessToken,
    accessToken,
  });
              ...
              <TopbarDropdownGroup>
                ...
                <TopbarDropdownItem asChild>
                  <Link
                    ...
                    className="flex gap-x-2"
                  >
                    <ComplicateIcons.Doc className="h-4 w-4" fillAreaColor="" />
