...
export const ClonePipelineDialog = ({
  ...
  async function handleClone(data: z.infer<typeof ClonePipelineSchema>) {
    if (!me.isSuccess || !entityObject.isSuccess || !accessToken || !pipeline) {
      return;
    }
    ...
    try {
      ...
      // Because this is the same route for nextjs, we need to indicate that
      // We need to reload the page to get the new data
      router.reload();
    } catch (error) {
      setCloning(false);
      toastInstillError({
