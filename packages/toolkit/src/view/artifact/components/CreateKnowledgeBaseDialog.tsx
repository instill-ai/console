...
export const CreateKnowledgeBaseFormSchema = z.object({
...
export const CreateKnowledgeBaseDialog = ({
  ...
  const form = useForm<z.infer<typeof CreateKnowledgeBaseFormSchema>>({
  ...
  const formattedName = formatResourceId(
    nameValue,
    resourceIdPrefix.knowledgeBase,
  );
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      router.push(`/${data.namespaceId}/knowledge-base`);
      onClose();
    } catch (error) {
      ...
      <Dialog.Content className="!w-[600px] rounded-md">
        <Dialog.Header className="flex justify-between">
          ...
          <Dialog.Description className="sr-only">
            Create a new knowledge base with name, description, and tags
          </Dialog.Description>
