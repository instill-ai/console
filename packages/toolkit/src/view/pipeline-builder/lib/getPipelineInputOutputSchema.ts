...
export function getPipelineInputOutputSchema(
  ...
  inputSchema = (
    (schema.paths["/trigger"]?.post?.requestBody as OpenAPIV3.RequestBodyObject)
      .content["application/json"]?.schema as OpenAPIV3.SchemaObject
  ).properties?.inputs as OpenAPIV3.ArraySchemaObject as InstillJSONSchema;
  ...
  ).properties?.outputs as OpenAPIV3.ArraySchemaObject as InstillJSONSchema;
