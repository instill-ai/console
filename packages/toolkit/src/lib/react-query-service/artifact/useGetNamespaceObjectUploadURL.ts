...
export function useGetNamespaceObjectUploadURL() {
  return useMutation({
    mutationFn: async ({
      ...
      const effectiveAccessToken = shareToken ? undefined : (accessToken ?? undefined);
      ...
      const sdkPayload = {
        parent: payload.parent || (payload.namespaceId ? `namespaces/${payload.namespaceId}` : ""),
      ...
      const response =
        await client.artifact.getNamespaceObjectUploadURL(sdkPayload as unknown as Parameters<typeof client.artifact.getNamespaceObjectUploadURL>[0]);
